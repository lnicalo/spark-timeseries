...
class TimeSeriesRDDSuite extends FunSuite with LocalSparkContext with ShouldMatchers {
  ...
  test("weighted mean") {
    ...
    val index = uniform(start, 10, 1.businessDays)
    val rdd = new TimeSeriesRDD[String](index, sc.parallelize(vecs))
    val w = new DenseVector((1 to 5).map(_.toDouble).toArray)
    val rdd_right = rdd.weightedAverage(w, "Right")
    val rdd_center = rdd.weightedAverage(w, "Center")
    val rdd_left = rdd.weightedAverage(w, "Left")
    ...
    val contents = rdd_left.collectAsMap()
    contents.size should be (3)
    contents("0.0") should be (new DenseVector((40 to 115 by 15).map(_.toDouble).toArray))
    contents("10.0") should be (new DenseVector((190 to 265 by 15).map(_.toDouble).toArray))
    contents("20.0") should be (new DenseVector((340 to 415 by 15).map(_.toDouble).toArray))
  }
}
